...
export default function MapView() {
  ...
  return (
    <div data-map-root className="relative -mx-4 -my-6 h-[calc(100vh-53px)] overflow-hidden bg-slate-200 sm:-mx-6">
      ...
      {/* États — centrés au milieu de la carte (visibles, hors des barres). */}
      <AnimatePresence>
        {loading && (
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            className="glass pointer-events-none absolute left-1/2 top-1/2 z-[660] flex -translate-x-1/2 -translate-y-1/2 items-center gap-2.5 rounded-full px-5 py-3 text-sm font-medium text-slate-700 shadow-lg">
            <svg className="h-4 w-4 animate-spin text-orange-500" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      {error && (
        <div className="absolute left-1/2 top-1/2 z-[660] max-w-[90%] -translate-x-1/2 -translate-y-1/2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm text-rose-700 shadow-lg">
          {error.message}
        </div>
      )}
