...
from parcelaire.models import (
    ...
    PropertyUnit,
    Reservation,
    SaleFile, ParcelTag, ProgramOrthophoto,
)
...
class RealEstateMapAPIView(APIView):
    ...
    def serialize_orthophoto(self, orthophoto):
        if not orthophoto:
            return None
        ...
        try:
            if orthophoto.bounds:
                bounds = json.loads(orthophoto.bounds.geojson)
        except Exception:
            bounds = None

        try:
            if orthophoto.centroid:
                centroid = [orthophoto.centroid.y, orthophoto.centroid.x]
        except Exception:
            centroid = None

        return {
            "id": orthophoto.id,
            "name": orthophoto.name or "",
            "slug": orthophoto.slug,
            "program_id": orthophoto.program_id,
            "program_name": getattr(orthophoto.program, "name", ""),
            "tiles_folder": orthophoto.tiles_folder,
            "tiles_url": orthophoto.tiles_url,
            "min_zoom": orthophoto.min_zoom,
            "max_zoom": orthophoto.max_zoom,
            "max_native_zoom": orthophoto.max_native_zoom,
            "capture_date": orthophoto.capture_date.isoformat() if getattr(orthophoto, "capture_date", None) else None,
            "reference_year": getattr(orthophoto, "reference_year", None),
            "reference_month": getattr(orthophoto, "reference_month", None),
            "period_label": getattr(orthophoto, "period_label", None),
            "version": getattr(orthophoto, "version", None),
            "is_current": bool(getattr(orthophoto, "is_current", False)),
            "is_active": bool(getattr(orthophoto, "is_active", False)),
            "bounds": bounds,
            "centroid": centroid,
    ...
    def _build_from_parcels(self, queryset, params):
        ...
        for parcel in queryset:
            ...
            assets_payload.append({
                ...
                "program": parcel.program.name if parcel.program else "—",
                "program_id": parcel.program_id if parcel.program_id else None,
                "orthophoto": self.get_program_current_orthophoto_payload(parcel.program) if parcel.program else None,
